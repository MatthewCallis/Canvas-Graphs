...
var Graph = function(options) {
	...
	this.doGraph = function(){
		...
		if(options['debug']){
			console.log("CALL: " + call + " VALUE: " + value + " SCALE: " + this.scale);
		}
	...
	this.drawFill = function(){
		...
		for(var i = 0; i < this.array.length; i++){
			...
			this.context.lineTo(i * this.grid_x + 0.5 + this.leftOffset, pos_y);
		}

		this.context.lineTo((i-1) * this.grid_x + 0.5 + this.leftOffset, this.canvas.height - 0.5);
	...
	this.drawLine = function(){
		...
		for(var i = 0; i < this.array.length; i++){
			var pos_y = this.array[i];
			this.context.lineTo(i * this.grid_x + offset + this.leftOffset, pos_y);
		}
		this.context.stroke();
		this.context.closePath();
	}
	this.drawDots = function(){
		...
		for(var i = 0; i < this.array.length; i++){
			...
			this.context.beginPath();
			this.context.arc(i * this.grid_x + 0.5 + this.leftOffset, pos_y - 0.5, dotradius, 0, Math.PI*2, true);
			this.context.fill();
			this.context.closePath();
		}
		this.context.closePath();
	...
	this.drawLabels = function(){
		for(var i = 0; i < this.array.length; i++){
			...
			this.drawLabel(text, i * this.grid_x + 0.5 + this.leftOffset, pos_y-0.5);
		}
	}
	...
	this.drawGrid = function(){
		...
		if(this.grid_x > 0){
			for (var x = 0.5; x < this.canvas.width; x += this.grid_x){
				this.context.moveTo(x + this.leftOffset, 0);
				this.context.lineTo(x + this.leftOffset, this.canvas.height);
			}
		}
		if(this.grid_y > 0){
	...
	if(this.data == undefined){
		...
		for(var i =0; i < this.data.length; i++){
			// Convert real values to graph values
			this.array[i] = Math.round((this.range[1] - this.data[i] ) * this.scale);
		}
		this.drawGraph();
	}
}

function csv_array(e,b){b=b||",";for(var f=RegExp("(\\"+b+'|\\r?\\n|\\r|^)(?:"([^"]*(?:""[^"]*)*)"|([^"\\'+b+"\\r\\n]*))","gi"),c=[[]],a=null;a=f.exec(e);){var d=a[1];d.length&&d!=b&&c.push([]);a=a[2]?a[2].replace(RegExp('""',"g"),'"'):a[3];c[c.length-1].push(a)}return c};